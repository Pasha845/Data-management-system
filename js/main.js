document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelector('.cart__form').addEventListener('input', () => {
  ...
  document.form.addEventListener('submit', function (event) {
    ...
    const form = this;
    const item = ['name'].map((name) => form[name].value);
    form.reset();
    studetsDate.push(item);
    localStorage.setItem('localData', JSON.stringify(studetsDate));
    buildTable(studetsDate);
  });
});